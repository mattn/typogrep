...
func doGrep(filename string) error {
	...
	for scanner.Scan() {
		...
		for _, word := range words {
			if _, ok := typos[word]; ok {
				re := regexp.MustCompile(`\b` + word + `\b`)
				ill := re.FindAllStringIndex(text, -1)
				if len(ill) > 0 {
					fmt.Fprintf(out, "%s:%d:", filename, nr)
					for i, il := range ill {
						if i > 0 {
							fmt.Fprint(out, text[ill[i-1][1]:il[0]]+cRED+text[il[0]:il[1]]+cRESET)
						} else {
							fmt.Fprint(out, text[0:il[0]]+cRED+text[il[0]:il[1]]+cRESET)
						}
					}
					fmt.Fprintln(out, text[ill[len(ill)-1][1]:])
					break
				}
			}
		}
...
func main() {
	flag.Parse()
	if flag.NArg() == 0 {
		if err := doGrep(""); err != nil {
			fmt.Fprintln(os.Stderr, err)
		}
		return
	}
	for _, arg := range flag.Args() {
		if err := doGrep(arg); err != nil {
			fmt.Fprintln(os.Stderr, err)
		}
	}
}
